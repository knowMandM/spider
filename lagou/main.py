    ...
    def getDataByCity(self, sSearch = '', sCity = '', gm = "", totalJobCount = 0):
        ...
        while x < (totalJobCount // 15 + 1):
            ...
            if jsonObj["content"]["positionResult"]['resultSize'] <= 0:
                log("没有记录")
                continue
            write_csv.writeInfo(jsonObj["content"]["positionResult"]["result"], sSearch, sCity, gm)
            if jsonObj["content"]["positionResult"]['resultSize'] < 15:
                return log("没有下一页了，跳出")
    # 功能入口
    def run(self):
        for searchStr in search_condition.searchArr:
            for city in search_condition.cityArr:
                jobCount = self.getJobCountInCity(searchStr, city)
                if(jobCount > 0):
                    if(jobCount < 3000):
                        self.getDataByCity(searchStr, sCity = city, totalJobCount=jobCount)
                    else:
                        for guimo in search_condition.gmArr:
                            self.getDataByCity(searchStr, city, guimo, self.getJobCountInCity(searchStr, city, guimo))
                else:
                    log(city, "没有", searchStr, "相关职位，跳过。")
    
    # 功能入口2, 目前使用这个
    def runEx(self):
        for searchStr in search_condition.searchArr:
            jobCount = self.getJobCountInCity(searchStr)
            if(jobCount > 0):
                self.getDataByCity(searchStr, totalJobCount = jobCount)
            else:
                log(searchStr, "没有相关职位")
